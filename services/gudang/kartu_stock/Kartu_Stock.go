	...
	for j := 0; j < len(read_sup); j++ {
		...
		for i := 0; i < len(raw_data); i++ {
			...
			if strings.HasPrefix(raw_data[i].Kode_stock_keluar_masuk, "SM") {
				...
				err = con_stock_raw.Select("stock_dalam_sistem, stock_rill").Joins("JOIN detail_audit da ON da.kode_audit=audit.kode_audit").Where("audit.kode_audit = ? && kode_supplier = ? && kode_stock = ? && audit.status = 1", raw_data[i].Kode_stock_keluar_masuk, read_sup[j].Kode_supplier, read_sup[j].Kode_stock).Scan(&audit_raw)
